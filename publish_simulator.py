...
def simulate_publish(publisher: Publisher, detection: P_Tactical_Sensor_PSM_C_Detection):
    ...
    while True:
        ...
        detection.A_timeOfDataGeneration.A_seconds = (time_ns() // NANOSECONDS_CONVERSION_TO_SECONDS)
...
def choose_random_id_or_enum() -> BigInteger:
    if random() < CHANCE_FOR_ID:
        returned_id = choice(list(Random16DigitID)).value
    else:
        returned_id = randint(TRUELY_RANDOM_16_DIGIT_ID_START_VAR, TRUELY_RANDOM_16_DIGIT_ID_END_VAR)
